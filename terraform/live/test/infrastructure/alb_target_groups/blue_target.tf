...
module "backend_blue_target_group" {
  source = "../../../../modules/aws/alb_target_group"

  vpc_id                                            = data.aws_ssm_parameter.vpc_id.value
  alb_target_group_name                             = local.env.app_backend_blue_alb_target_group_name
  ...
  alb_target_group_health_check_timeout             = 15
  ...
  ssm_alb_target_group_arn_key                      = local.env.ssm_params.app_backend_blue_alb_target_group_arn
  tags                                              = {}
}